...
test.describe("Доступность: ориентиры, заголовки, клавиатура", () => {
  ...
  test.fixme(
    "экран карты имеет заголовок первого уровня",
    async () => {
      /*
        ОШИБКА ДОСТУПНОСТИ В ПРИЛОЖЕНИИ.

        `MapScreen` не использует `PageHeader` и не содержит ни одного `h1`:
        на /map единственные заголовки — `h2` панели слоёв. Пользователь
        скринридера, перейдя на карту, не услышит названия страницы, а
        навигация по заголовкам начнётся сразу со второго уровня.

        Правка в `frontend/src/` в этой задаче не предусмотрена, поэтому
        проверка отключена, а не ослаблена до «хоть какой-нибудь заголовок».
      */
    },
  );
});
