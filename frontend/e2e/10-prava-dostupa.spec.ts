...
test.describe("Сценарий 10. Роль «Просмотр» и разграничение прав", () => {
  ...
  test.fixme(
    "роль «Просмотр» не видит в меню разделов, требующих прав",
    async () => {
      /*
        НЕ РЕАЛИЗОВАНО В ПРИЛОЖЕНИИ.

        Боковое меню (`src/components/layout/Sidebar.tsx`) отдаёт один и тот же
        список из шести пунктов всем: «Дашборд», «Карта», «Данные (импорт)»,
        «Отчёты», «Граф связей», «Администрирование». Роль нигде не читается —
        в компоненте нет ни `readUser`, ни проверки прав.

        Данные при этом не утекают: сервер отказывает, и экраны показывают
        «Недостаточно прав» (проверено выше). Но пользователю роли «Просмотр»
        предлагаются четыре раздела, в которых он получит отказ, — это ошибка
        интерфейса, а не безопасности.

        Сюда же относится верхняя панель: `AppShell` не передаёт `Topbar`
        сведения о пользователе, и вошедший всегда подписан «Гость / Просмотр»
        независимо от того, кто он на самом деле.
      */
    },
  );
});
