...
for (const viewport of VIEWPORTS) {
  test.describe(`Адаптивность: ${viewport.name}`, () => {
    ...
    if (viewport.width >= 1024) {
      ...
      test("вариант «Карта + список» недоступен на узком экране", async ({ page }) => {
        /*
          Две колонки на телефон не помещаются, поэтому режим не просто
          прячется правилом CSS, а исключается из набора: иначе он остаётся в
          обходе с клавиатуры и его можно выбрать стрелками, получив
          раскладку, которая не поместится.

          Раньше он прятался классами `hidden lg:inline-flex`, но обе утилиты
          задают `display`, и в собранном CSS побеждала вторая — кнопка была
          видима и фокусируема при ширине 390 px.
        */
        await signIn(page, "analyst");
        await page.goto("/map?view=map");

        await expect(page.getByRole("radio", { name: "На карте" })).toBeVisible();
        await expect(page.getByRole("radio", { name: "Карта + список" })).toHaveCount(0);
      });
    }
