...
export function AuthGuard({ children }: { children: React.ReactNode }) {
  ...
  */
  const [state] = useState<"unknown" | "authorized">(() =>
    // Ленивая проверка при первой отрисовке вместо записи состояния в
    // эффекте: запись в эффекте вызывает лишний каскад перерисовок.
    // На сервере `readToken` возвращает null, и там состояние остаётся
    // «неизвестно» — это верно, сервер и правда не знает.
    readToken() ? "authorized" : "unknown",
  );
