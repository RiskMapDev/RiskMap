...
const TOP_DISTRICTS = [
  { name: 'Карасайский р-н', score: 87, color: '#dc2626' },
  { name: 'Талгарский р-н', score: 74, color: '#f59e0b' },
  { name: 'Илийский р-н', score: 68, color: '#f59e0b' },
  { name: 'Енбекшиказахский р-н', score: 61, color: '#f59e0b' },
  { name: 'Жамбылский р-н', score: 55, color: '#f59e0b' },
  { name: 'Балхашский р-н', score: 48, color: '#22c55e' },
  { name: 'Уйгурский р-н', score: 42, color: '#22c55e' },
]
        ...
        <div style={{ flex: 1, position: 'relative', minWidth: 0 }}>
          ...
          <div style={{ position: 'absolute', top: 12, right: 12, bottom: 12, width: 280, background: '#fff', borderRadius: 12, boxShadow: '0 4px 24px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', zIndex: 1000, overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
              ...
              {tab === 'risks' && (
                <>
                  <div style={{ fontSize: 11, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 12 }}>Объекты с высоким риском</div>
                  {[
                    { name: 'Школа №12 — капремонт', district: 'Карасайский р-н', risk: 'Критический', amount: '124.5 млн' },
                    { name: 'Водопровод с. Отеген', district: 'Илийский р-н', risk: 'Критический', amount: '89.2 млн' },
                    { name: 'ФАП с. Бесагаш', district: 'Талгарский р-н', risk: 'Высокий', amount: '42.0 млн' },
                    { name: 'ДГЗ-2026-1601 Субсидии', district: 'УСХ Алматинской обл.', risk: 'Высокий', amount: '156.4 млн' },
                    { name: 'Дорога Кегень—Текес', district: 'Райымбекский р-н', risk: 'Высокий', amount: '312.8 млн' },
                    { name: 'Газопровод с. Шелек', district: 'Енбекшиказахский р-н', risk: 'Высокий', amount: '78.3 млн' },
                  ].map((o, i) => {
                    const isCrit = o.risk === 'Критический'
                    return (
                      <div key={i} style={{ padding: '10px 0', borderBottom: '1px solid #f1f5f9' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            <div style={{ fontSize: 11, color: '#94a3b8' }}>{o.district}</div>
                          </div>
                          <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 20, flexShrink: 0, background: isCrit ? '#fef2f2' : '#fff7ed', color: isCrit ? '#dc2626' : '#ea580c' }}>{o.risk}</span>
                        </div>
                        <div style={{ fontSize: 11, color: '#64748b', marginTop: 4 }}>Сумма: ₸{o.amount}</div>
                      </div>
                    )
                  })}
                </>
              ...
              {tab === 'top' && (
                <>
                  <div style={{ fontSize: 11, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 12 }}>Рейтинг районов по риску</div>
                  ...
                  <div style={{ marginTop: 20 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 10 }}>Топ поставщики по рискам</div>
                    {[
                      { name: 'ТОО КазСтройИнвест', contracts: 8, risk: 'Высокий' },
                      { name: 'ТОО РегионСтрой', contracts: 5, risk: 'Средний' },
                      { name: 'ТОО АлматыЖолСервис', contracts: 6, risk: 'Высокий' },
                    ].map((s, i) => (
                      <div key={i} style={{ padding: '8px 0', borderBottom: '1px solid #f1f5f9' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          ...
                          <span style={{ fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 20, background: s.risk === 'Высокий' ? '#fef2f2' : '#fff7ed', color: s.risk === 'Высокий' ? '#dc2626' : '#ea580c' }}>{s.risk}</span>
                        </div>
                        <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 2 }}>{s.contracts} договоров</div>
                      </div>
                    ))}
                  </div>
                </>
